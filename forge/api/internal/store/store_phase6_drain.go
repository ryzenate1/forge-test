...
func (s *Store) UpsertDrainState(ctx context.Context, state DrainState) (DrainState, error) {
	execErr := s.db.Exec(ctx, `
		...
		ON CONFLICT (node_id) DO UPDATE SET
			...
			updated_at = now()
	`, state.NodeID, nullIfEmpty(state.PlanID), state.Status, state.DesiredFinal, state.StartedAt, state.CompletedAt, string(state.Progress.asJSON()))
	if execErr != nil {
		return DrainState{}, execErr
	}
