...
const field = "mt-1 h-10 w-full rounded-lg border border-white/10 bg-[var(--surface)] px-3 text-sm text-white outline-none focus:border-red-500";
...
export function DatabasesView({ server }: { server?: ApiServer }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="space-y-3">{query.isLoading ? <div className="rounded-xl bg-[var(--surface-raised)] p-5 text-sm text-slate-400">Loading databases…</div> : null}{!query.isLoading && !query.isError && rows.length === 0 ? <div className="rounded-xl bg-[var(--surface-raised)] p-8 text-center text-sm text-slate-400">No databases have been created.</div> : null}{rows.map((row) => {
