...
export function FilesView({ server }: { server?: ApiServer }) {
  ...
  const openFile = async (path: string) => { setEditing(path); setContent(""); setFileLoaded(false); setDirty(false); setError(""); setStatus("Loading"); try { const value = await readServerFile(server?.id ?? "", path); setContent(value); setInitialContent(value); setDirty(false); setFileLoaded(true); setStatus("Loaded"); } catch (loadError) { setError(errorMessage(loadError, "File could not be loaded.")); setStatus("Load failed"); } };
  ...
  if (editing) return <div className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3"><Breadcrumbs directory={editing} onOpen={requestCloseEditor} /><div className="flex items-center gap-2"><span className="text-xs text-slate-400" role="status">{status}{dirty ? " · unsaved changes" : ""}</span><button className={button} onClick={() => requestCloseEditor(null)} type="button">Close</button><button className="inline-flex h-9 items-center gap-2 rounded-lg bg-red-600 px-4 text-xs font-bold text-white disabled:opacity-40" disabled={!canUpdate || !fileLoaded || busy} onClick={() => void save()} type="button"><Save size={15} />Save content</button></div></div>{error ? <div className="rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-200" role="alert">{error}</div> : null}<div className="h-[65vh] min-h-96 overflow-hidden rounded-xl border border-white/10 bg-[#111722]"><MonacoEditor language={languageFor(editing)} onChange={(value) => { if (fileLoaded) { setContent(value ?? ""); setDirty(value !== initialContent); setStatus(value === initialContent ? "Loaded" : "Edited"); } }} options={{ fontSize: 14, lineNumbers: "on", minimap: { enabled: false }, readOnly: !fileLoaded || !canUpdate, wordWrap: "on", automaticLayout: true }} theme="vs-dark" value={content} /></div><ConfirmDialog danger open={discardTarget} title="Discard unsaved changes?" description={`Your changes to ${editing} have not been saved and will be lost.`} confirmLabel="Discard changes" cancelLabel="Keep editing" onClose={() => { setDiscardTarget(false); setPendingNav(null); }} onConfirm={confirmDiscard} /></div>;
