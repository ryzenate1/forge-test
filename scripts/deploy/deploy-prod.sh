...
if [ "$ROLLBACK" = true ]; then
  ...
  # Find the newest regular backup without parsing `ls` output.
  LATEST_BACKUP=""
  shopt -s nullglob
  for candidate in "$ROLLBACK_DIR"/.env.*; do
    if [[ -f "$candidate" && ! -L "$candidate" ]] && { [[ -z "$LATEST_BACKUP" ]] || [[ "$candidate" -nt "$LATEST_BACKUP" ]]; }; then
...
if [ "$DRY_RUN" = false ]; then
  (cd "$COMPOSE_DIR" && TAG=$VERSION docker compose $COMPOSE_FILES $ENV_FILE up -d --remove-orphans) || fail "Deploy failed"
