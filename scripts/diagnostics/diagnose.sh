...
DIAG_DIR=$(mktemp -d)
chmod 700 "$DIAG_DIR"
REPORT="$DIAG_DIR/diagnostic-report-$(date +%Y%m%d-%H%M%S).txt"
...
if [ -d "$SCRIPT_DIR/../../infra" ]; then
  (cd "$SCRIPT_DIR/../../infra" && docker compose -f compose.yml ps 2>/dev/null) | tee -a "$REPORT" || info "Docker Compose not available"
fi
